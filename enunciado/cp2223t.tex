...
A função \text{\ttfamily gene} vai ser espressa em função do seu caso de paragem e caso geral. De notar que, se aplicarmos o funtor das listas não vazias (\text{\ttfamily out}) ao argumento da função, podemos definir o gene como uma soma de funções.
...
Deste modo, começa-se por um \text{\ttfamily map~trim} à lista inicial. A função \text{\ttfamily trim} remove os primeiro quatro espaços de uma \text{\ttfamily string}. De seguida, essa função é composta com um \text{\ttfamily groupBy~\char40{}const~canTrim\char41{}}\footnote{Dada a assinatura da função \text{\ttfamily groupBy}, houve a necessidade de adicionar um primeiro argumento à função \text{\ttfamily canTrim}, que deve ser ignorado pela mesma, daí a utilização da função \text{\ttfamily const}}. A função \text{\ttfamily groupBy} está definida no módulo de Haskell \text{\ttfamily Data\char46{}List} e parte uma lista sempre que a função argumento seja verdadeiro, colocando o elemento para o qual isso aconteceu à cabeça de uma nova lista.
...
\\[\blanklineskip]%
\>[B]{}\Varid{carpets}\;\Varid{n}\mathrel{=}\Varid{map}\;(\Varid{sierpinski}\comp \conj{\underline{\Varid{defaultSquare}}}{\Varid{id}})\;[\mskip1.5mu \mathrm{0}\mathinner{\ldotp\ldotp}\Varid{n}\mskip1.5mu]{}\<[E]%
\\[\blanklineskip]%
...
\\[\blanklineskip]%
\>[B]{}\Varid{sideSquares}\;(\Varid{xy},\Varid{l})\mathrel{=}\Varid{map}\;\conj{(\Varid{addTup}\;\Varid{xy})\comp (\Varid{scaleTup}\;\Varid{l'})}{\underline{\Varid{l'}}}\;[\mskip1.5mu (\mathrm{1},\mathrm{2}),(\mathrm{2},\mathrm{2}),(\mathrm{2},\mathrm{1}),(\mathrm{2},\mathrm{0}),(\mathrm{1},\mathrm{0}),(\mathrm{0},\mathrm{0}),(\mathrm{0},\mathrm{1}),(\mathrm{0},\mathrm{2})\mskip1.5mu]{}\<[E]%
\\
\>[B]{}\hsindent{5}{}\<[5]%
...
Derivamos, deste modo, o tipo da função \text{\ttfamily gr2l} pretendida. O que se pretende é que esta função concatene toda as listas que recebe como argumento no segundo elemento do par, e que adicione o elemento que recebe como primeiro elemento do par à cabeça da lista. 
...
Pretende aplicar-se, então, a função \text{\ttfamily sierpinski} com todos os limites de profundidade de 0 a $n$ (em que $n$ é o parâmetro de \text{\ttfamily carpets}). Isso motiva a aplicar um \text{\ttfamily map} da função \text{\ttfamily sierpinski} à lista \text{\ttfamily \char91{}0\char46{}\char46{}n\char93{}}:
...
A definição da função \text{\ttfamily pinitKnockoutStage} é trivial quando feita à custa da função \text{\ttfamily initKnockoutStage}. De facto, a \text{\ttfamily pinitKnockoutStage} não depende em nada de probabilidades; a única diferença em relação à sua variante não monádica é o facto de ter de retornar um valor no monáde das probabilidades. Assim sendo, basta utilizar a função \text{\ttfamily return} do monáde após aplicar a \text{\ttfamily initKnockoutStage} original; ficando assim definida como
...
A segunda parte consiste em, sabendo os resultados dos jogos, determinar os vencedores do grupo. Para isso, é necessário juntar as listas correspondentes a cada jogo numa só - usando, para isso, a função \text{\ttfamily concat} - e consolidar (\text{\ttfamily consolidate}) a lista, de forma a ficar com a pontuação de cada equipa. Finalmente, é necessário retornar os 2 melhores, utilizando, para isso, a função \text{\ttfamily best~2}. Como estamos a trabalhar num contexto monádico, é necessário utilizar a função \text{\ttfamily return} do monáde das distribuições de probabilidade para \textit{colocar} o resultado dentro desse monáde.

De forma a combinar estas duas partes, utiliza-se o operador \text{\ttfamily \char62{}\char62{}\char61{}} de Haskell, para passar o resultado da primeira para a segunda. Assim sendo, o resultado é
