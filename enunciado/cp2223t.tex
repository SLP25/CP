...
&
     \ensuremath{\Conid{String}} + \ensuremath{\Conid{String}} \times \ensuremath{\Conid{String}}^{*^{*}}
          \ar[u]^{\ensuremath{\Varid{id}\mathbin{+}(\Varid{id}\times(\lambda \Varid{map}\;\lanabracket\;\!\Varid{gene}\;\!\ranabracket_\textit{\tiny Exp}))}}
}
\end{eqnarray*}

A função \text{\ttfamily gene} vai ser expressa em função do seu caso de paragem e caso geral. De notar que, se aplicarmos o funtor das listas não vazias (\text{\ttfamily out}) ao argumento da função, podemos definir o gene como uma soma de funções.
